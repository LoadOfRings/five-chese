...
int ServerPort=4000;
char ServerIPAddr[50]="210.30.105.29";
...
void Set_SendData(int data,char buffer[]);
//小鳖的AI
...
void network_game(){	
	...
	while(true){
		...
		ServerAddr.sin_addr.S_un.S_addr=inet_addr(ServerIPAddr);
		...
		if((ReceiveLen=recv(ClientSocket,
			...
			xy[sum/100][sum%100]=-1;
			//printf("receive Data:%s",ReceiveBuffer);
		}
		flag=check();
		if(flag==1){
			system("cls");
